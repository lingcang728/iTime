...
每完成一轮仓库修改，都必须同步推进应用版本号，禁止继续复用上一轮版本号：

- 普通修复或小功能递增补丁号，例如 `0.1.1` → `0.1.2` → `0.1.3`。
- 重大功能、架构或数据边界调整递增次版本号，例如 `0.1.1` → `0.2.0`。
- 每轮发布前必须确保 `package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock`、`src-tauri/tauri.conf.json`、发布清单、安装包文件名、Git Tag 与 GitHub Release 版本完全一致。
- 当前轮次如用户指定了目标版本，以用户指定版本为准；本轮三项重大功能统一发布为 `0.2.0`。
