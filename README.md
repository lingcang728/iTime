...
**iTime** 是一款 Windows 本地桌面应用：自动记录前台应用占用，单独标出 AI 工具的前台时间，用首页、时间线、周报帮你看清节奏。活动、键盘计数和会话原始数据只留在你的电脑上，不会上传。
...
- 🔒 **本地优先** — 原始活动留在本机；不读窗口标题、文档名、对话和具体按键内容
...
活动记录保存在本机数据目录。设置页可以打开数据目录、导出 JSON/CSV、选择永久/365 天/90 天保留期，或在二次确认后清空。不会上传到云端，也没有账号云同步。遇到问题请直接在 GitHub Issue 里说明系统版本和复现步骤。
...
- ✅ 数据默认只在你电脑上
- ✅ “AI Agent 编程工具”单一开关统一授权 25 类工具，只解析会话 ID、时间和事件类型元数据
- ✅ 支持 JSON/CSV 导出、保留期和二次确认清空
- ❌ 不读窗口标题 / 文档名 / 聊天正文 / 提示词 / 具体按键
- ❌ 未授权时不扫描任何 AI Agent 工具目录
- ❌ 不上传任何使用数据、设备信息、提示词、回复、代码、工具输出、项目路径、用户名、序列号、MAC、窗口标题、按键内容或完整应用列表
- 🛡️ 应用身份经本地处理后再使用
- ⌨️ 键盘计数依赖 Windows 低级钩子（`WH_KEYBOARD_LL`）：它**能观察到全系统每次击键**，这是 Windows 上可用的最低权限计数原语；实现只判断"是否字符键"并累计数量，键值从未被保存或外发（有自动化测试断言），合成注入的击键一律忽略
- ℹ️ WebView2 运行时自身的网络行为（如 SmartScreen）属 Windows 组件行为，非本应用代码；卸载时数据目录可能随安装目录一并删除，请先自行备份导出文件

你可以把它当成 **「原始记录只在本机」** 的时间记账本。遇到问题请手动报备系统版本和复现步骤。
