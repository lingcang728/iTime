...
**iTime** 是一款 Windows 本地桌面应用：自动记录前台应用占用，单独标出 AI 工具的前台时间，用首页、时间线、周报帮你看清节奏。活动、键盘计数和会话原始数据只留在你的电脑上；只有你明确开启“AI Agent 编程工具”后，才会匿名上报有限的设备、性能和每日工具汇总。
...
活动记录保存在本机数据目录。设置页可以打开数据目录、导出 JSON/CSV、选择永久/365 天/90 天保留期，或在二次确认后清空。默认不会上传；只有你明确开启“AI Agent 编程工具”后，iTime 才会匿名上报电脑型号、CPU、GPU、内存、Windows/iTime 版本、少量性能指标，以及每个工具每天的任务数、执行秒数与最大并发。没有账号云同步。
...
- ✅ 数据默认只在你电脑上
- ✅ “AI Agent 编程工具”单一开关统一授权八类工具，只解析会话 ID、时间和事件类型元数据
- ✅ 授权后只匿名上报硬件、性能与每日工具汇总；匿名设备数据滚动保留 180 天
- ✅ 支持 JSON/CSV 导出、保留期和二次确认清空
- ❌ 不读窗口标题 / 文档名 / 聊天正文 / 提示词 / 具体按键
- ❌ 未授权时不扫描任何 AI Agent 工具目录、不采集硬件、不产生上传队列
- ❌ 不上传提示词、回复、代码、工具输出、项目路径、用户名、序列号、MAC、窗口标题、按键内容或完整应用列表
- 🛡️ 应用身份经本地处理后再使用

你可以把它当成 **「原始记录只在本机、匿名诊断明确授权」** 的时间记账本。
