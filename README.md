...
- 🤖 **单独看 AI** — 分开呈现 AI 前台观察与 Codex / Claude Code Provider 执行证据
- 📅 **时间线 + 周报** — 按时间回看切换轨迹，用一周趋势复盘
- ⌨️ **输入足迹** — 展示输入总量、日级键位热力、节奏与快捷键排行（不会保存输入内容）
- 🎯 **目标与提醒** — 为自己设定使用边界，而不是被算法推着走
- 🔒 **本地优先** — 不上传使用数据；不读窗口标题、文档名、对话或输入文本
...
- AI 前台活跃来自 Windows 活动采集；Provider 执行来自本机会话时间事件
- 显示累计执行、自然时间覆盖、并发与杠杆率；缺少任一证据时明确降级，**不用假数据凑满**
...
- 字符键分钟总量、活跃输入分钟、估算 WPM 与退格修正率
- 键位和快捷键只按本地日期累计；升级前日期不生成模拟热力分布
- 活动记录总开关会同时启停分钟总量与日级键位统计
...
| 🤖 **AI 有两类证据** | 前台观察不等于后台执行；Provider 指标只使用可读取的本机会话时间事件 |
| ⌨️ **输入只有聚合** | 分钟总量与日级键位/快捷键计数保存在本机，升级前逐键历史不会补造 |
...
不会。iTime **不**读取或保存窗口标题、文档名、对话内容或输入文本；键位与快捷键只按本地日期累计，不保存按键顺序、逐键时间戳、原始虚拟键码、IME 结果或剪贴板内容。应用也**不**保存完整程序路径、用户名等敏感标识。
...
- ✅ 数据默认只在你电脑上
- ✅ 只保存字符键分钟总量，以及按本地日期累计的规范化键位和快捷键次数
- ❌ 不读窗口标题 / 文档名 / 聊天与输入文本
- ❌ 不保存按键顺序 / 逐键时间戳 / 原始虚拟键码 / IME 结果 / 剪贴板内容
