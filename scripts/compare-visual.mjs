...
if (!skipReference && fs.existsSync(referencePath)) {
  ...
  for (const [page, rect] of Object.entries(rects)) {
    const actual = read(path.join(output, `reference-${page}.png`))
...
if (fs.existsSync(baseline)) {
  for (const file of fs.readdirSync(baseline).filter((value) => value.endsWith('.png'))) {
