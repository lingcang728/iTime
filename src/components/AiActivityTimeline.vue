...
function segment(id: string, kind: SegmentKind, interval: TimeRange, label: string): SegmentView {
  ...
  return { ...interval, id, kind, label, left: `${left}%`, width: `${Math.max(.45, right - left)}%` }
}
...
const lanes = computed<LaneView[]>(() => {
  ...
  const tools = props.tools.map((tool): LaneView => {
    const work = tool.workIntervals.map((item) => segment(item.id, 'work', item, `有效执行 · ${timeLabel(item)}`))
    const interactions = tool.interactionIntervals.map((item) => segment(item.id, 'interaction', item, `前台交互（估算） · ${timeLabel(item)}`))
...
<template>
  <div class="timeline" aria-label="AI 工具活动时间线">
    ...
    <div v-if="lanes.length === 1" class="timeline__empty">当天尚未采集到 AI 工具执行区间。</div>
    ...
    <div class="timeline__legend" aria-label="图例">
      <span class="is-human">人的前台活动</span><span class="is-work">有效执行</span>
      <span class="is-interaction">前台交互（估算）</span><span class="is-wait">静默等待</span><span class="is-overlap">并行重叠</span>
    </div>
  </div>
</template>
...
.timeline__axis { align-items: end; height: 24px; color: var(--text-muted); font-size: 8px; }
...
.timeline__name strong { overflow: hidden; color: var(--text-primary); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
.timeline__name span { color: var(--text-muted); font-size: 7px; }
.timeline__track {
  height: 27px;
  position: relative;
  border-radius: 7px;
...
.timeline__legend { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 12px; margin-top: 9px; color: var(--text-muted); font-size: 8px; }
...
.timeline__empty { margin: 18px 0 8px 124px; color: var(--text-muted); font-size: 9px; }
