...
<template>
  <div class="achievements">
    <article v-for="item in achievements" :key="item.id" :class="{ unlocked: item.unlocked, unavailable: !item.available }">
      <span class="achievements__icon"><PhCheck v-if="item.unlocked" :size="15" weight="bold" aria-hidden="true" /><PhLockSimple v-else :size="14" aria-hidden="true" /></span>
...
.achievements__copy strong { color: var(--text-primary); font-size: var(--text-micro); font-weight: 680; }
.achievements__copy small { overflow: hidden; color: var(--text-muted); font-size: var(--text-micro); text-overflow: ellipsis; white-space: nowrap; }
.achievements em { color: var(--text-muted); font-size: var(--text-micro); font-style: normal; white-space: nowrap; }
