...
<template>
  <ol v-if="apps.length" class="weekly-apps">
    <li v-for="(app, index) in apps" :key="app.appId">
      ...
      <ApplicationIcon :app-identity="app.appId" :app-name="app.appName" :size="24" />
...
.weekly-apps li { min-width: 0; min-height: 42px; display: grid; grid-template-columns: 18px 24px minmax(84px, 1fr) minmax(56px, .8fr) auto; align-items: center; gap: 8px; border-bottom: 1px solid var(--border-soft); }
...
@media (max-width: 760px) {
  ...
  .weekly-apps li { grid-template-columns: 18px 24px minmax(84px, 1fr) minmax(52px, .65fr) auto; }
}
</style>
