...
import { PhCheckCircle, PhClockCounterClockwise, PhInfo, PhX } from '@phosphor-icons/vue'
    ...
    <article class="card full-timeline" aria-labelledby="activity-tracks-title">
      <header class="track-header">
        ...
        <div class="track-actions">
          ...
          <button type="button" class="timeline-info-button" aria-label="查看统计口径与轨道说明" aria-controls="timeline-notes" :aria-expanded="notesOpen" @click="notesOpen = !notesOpen" @keydown.escape="notesOpen = false"><PhInfo :size="16" weight="fill" /></button>
      ...
      <div class="timeline-legend" aria-label="时间线颜色说明">
        <span><i class="green" />设备活跃</span><span><i class="idle" />设备空闲</span><span><i class="unknown" />状态未知</span><span><i class="blue" />前台应用</span><span><i class="violet" />AI 前台</span><span><i class="orange" />媒体播放</span>
      </div>
    </article>
...
.full-timeline { position: relative; margin-top: 12px; padding: 20px 21px 18px; }
...
.track-actions { position: relative; display: flex; align-items: center; gap: 7px; }
.track-actions > span { display: inline-flex; align-items: center; gap: 5px; flex: 0 0 auto; padding: 5px 8px; border-radius: 999px; color: var(--accent-green-strong); background: var(--accent-green-soft); font-size: 10px; }
.timeline-info-button { width: 28px; height: 28px; display: grid; place-items: center; padding: 0; border: 1px solid var(--border-soft); border-radius: 50%; color: var(--text-secondary); background: var(--bg-soft); cursor: pointer; transition: color 160ms ease, background 160ms ease, transform 160ms var(--ease-out); }
.timeline-info-button:hover { color: var(--accent-green-strong); background: var(--accent-green-soft); }
.timeline-info-button:active { transform: scale(.94); }
.timeline-popover { width: min(370px, calc(100vw - 48px)); position: absolute; z-index: 6; top: 36px; right: 0; display: grid; gap: 12px; padding: 14px 15px; border: 1px solid var(--border-strong); border-radius: 12px; background: color-mix(in srgb, var(--bg-elevated) 96%, transparent); box-shadow: 0 16px 38px rgba(0, 0, 0, .22), inset 0 1px 0 rgba(255, 255, 255, .035); }
.timeline-popover > button { position: absolute; top: 8px; right: 8px; width: 26px; height: 26px; display: grid; place-items: center; padding: 0; border: 0; border-radius: 7px; color: var(--text-muted); background: transparent; cursor: pointer; }
.timeline-popover > button:hover { color: var(--text-primary); background: var(--bg-hover); }
.timeline-popover > div { display: grid; gap: 3px; padding-right: 22px; }
.timeline-popover span { color: var(--accent-green-strong); font-size: 9px; font-weight: 700; letter-spacing: .06em; }
.timeline-popover strong { font-size: 12px; }
.timeline-popover p { margin: 2px 0 0; color: var(--text-secondary); font-size: 10px; line-height: 1.58; }
...
.timeline-tracks { display: grid; gap: 10px; padding: 2px 0; }
...
@media (max-width: 760px) { .timeline-overview { grid-template-columns: 1fr; } .timeline-axis { grid-template-columns: 72px repeat(7, 1fr); gap: 8px; } .track-actions > span { display: none; } }
</style>
